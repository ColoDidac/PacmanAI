...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        total_score = 0
        for x, y in curr_food:
            d = manhattanDistance((x, y), newPos)
            total_score += 2000 if d == 0 else 1 / (d ** 2)
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        beta = float("inf")
        alpha = float("-inf")
        return self.max_action(gameState, agent=0, depth=self.depth, alpha=alpha, beta=beta)

    def max_action(self, gameState, agent, depth, alpha, beta):
        v = float("-inf")
        actions = []
        for a in gameState.getLegalActions(agent):
            succ = gameState.getNextState(agent, action=a)
            u = self.min_value(gameState=succ, agent=1, depth=depth, alpha=alpha, beta=beta)
            if u > v:
                v = u
                actions.append(a)
            if v > beta:
    ...
    def max_value(self, gameState, agent, depth, alpha, beta):
        ...
        for a in gameState.getLegalActions(agent):
            succ = gameState.getNextState(agent, action=a)
            v = max(
                v, self.min_value(gameState=succ, agent=1, depth=depth, alpha=alpha, beta=beta)
            )
            if v > beta:
                return v
            alpha = max(alpha, v)
    ...
    def min_value(self, gameState, agent, depth, alpha, beta):
        ...
        for a in gameState.getLegalActions(agent):
            ...
            if agent == gameState.getNumAgents() - 1:
                v = min(
                    v, self.max_value(succ, agent=0, depth=depth - 1, alpha=alpha, beta=beta)
                )
            else:
                v = min(
                    v, self.min_value(succ, agent=agent + 1, depth=depth, alpha=alpha, beta=beta)
                )
            if v < alpha:
                return v
            beta = min(beta, v)
